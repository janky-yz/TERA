...
read_file=paste0(Align_dir, prefix, "_Log.final.out")
...
read=read.table(read_file, stringsAsFactors=F, sep='\t', fill=T)
...
FPK <- expr$expected_count/(expr$length/1000)
expr$FPKM <- FPK/as.numeric(read[5,2])*1000000
final=merge(bed, expr, all.x=T, by='transcript_id', sort=F)

keep <- final$expected_count>count
final[,14] <- paste0(final[,14],";FPKM=",final$FPKM)
