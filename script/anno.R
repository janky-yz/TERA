...
TE_element <- unique(overlap[,c(1:8,ncol(overlap))])
...
element_file = paste0(sample, ".TE.unit.anno.txt")
...
TE_exon <- unique(overlap[,c(9:16,ncol(overlap))])
names(TE_exon) <- c("chrom", "start", "end", "tid", "gid", "strand", "exon_type", "exon_class", "exon_ID")

TE_exon$type = "Annotated"
TE_exon[TE_exon$exon_ID %in% chimeric_exon_ID,]$type = "Novel chimeric"
